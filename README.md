...
1. [Pattern Algoritm](pattern_algoritm.md)   ← шаблоны паттернов с кодом на Go (справочник)
2. [Vizualization Algoritm](vizualization.md)      ← ASCII-визуализация, как паттерны работают внутри
3. [Learn Algoritm](learn_algoritm.md)     ← теоретическая база: Big-O, структуры, метод решения
4. [Leetcode list task Algoritm](leetcode_list_check.md) ← чек-лист задач с галочками
+  [Dictionary Algoritm](dictionary.md)       ← словарь терминов, открывается параллельно
+  roadmap (в README)    ← план по неделям

^ Логика применения такая: сначала учишь теорию (learn_algoritm), потом на каждой неделе roadmap открываешь пару «визуализация + шаблон» по нужному паттерну, решаешь задачи из чек-листа, а словарь держишь открытым фоном. Сейчас приведу README в порядок и всё это пропишу явно.
